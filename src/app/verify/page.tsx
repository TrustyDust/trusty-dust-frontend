...
export default function VerifyPage() {
  ...
  const [analyzeResult, setAnalyzeResult] =
    useState<WalletReputationResponse | null>(null)
  ...
  const handleAddressChange = (value: string) => {
    ...
    if (!isValidEthereumAddress(value)) {
      if (value.trim().length < 42) {
        setAddressError(
          "Wallet address must be 42 characters (0x + 40 hex characters)"
        )
      } else if (!value.trim().startsWith("0x")) {
        setAddressError("Wallet address must start with '0x'")
      } else if (!/^0x[a-fA-F0-9]*$/.test(value.trim())) {
        setAddressError(
          "Wallet address contains invalid characters (only 0-9, a-f, A-F allowed)"
        )
      } else {
  ...
  const { displayedText: reasoningText, isTyping: isTypingReasoning } =
    useTypingEffect(
                  ...
                  {(!analyzeResult || isLoading) && (
                    <section className="rounded-[28px] border border-white/10 bg-[#040f25]/80 p-6 backdrop-blur">
                      ...
                      <p className="mt-2 text-sm text-gray-400">
                        Create a zero-knowledge proof that verifies your
                        reputation tier without exposing your exact score or
                        transaction history.
                      </p>
                        ...
                        <div className="flex flex-1 flex-col justify-center rounded-[24px] border border-white/5 bg-[#050f22]/60 p-6">
                          ...
                          <div className="mt-3 h-2 rounded-full bg-white/10">
                            <div
                              ...
                              style={{
                                width: `${(analyzeResult.score / 1000) * 100}%`,
                              }}
                            />
                          </div>
                        ...
                        >
                          ...
                          <div className="mt-2 h-2 rounded-full bg-white/5">
                            <div
                              ...
                              style={{
                                width: visibleItems.has(0)
                                  ? `${
                                      (analyzeResult.breakdown.txnScore /
                                        1000) *
                                      100
                                    }%`
                                  : "0%",
                        ...
                        >
                          ...
                          <div className="mt-2 h-2 rounded-full bg-white/5">
                            <div
                              ...
                              style={{
                                width: visibleItems.has(1)
                                  ? `${
                                      (analyzeResult.breakdown.tokenScore /
                                        1000) *
                                      100
                                    }%`
                                  : "0%",
                        ...
                        >
                          ...
                          <div className="mt-2 h-2 rounded-full bg-white/5">
                            <div
                              className="h-full rounded-full bg-gradient-to-r from-[#2E7FFF] to-[#6B4DFF] transition-all duration-700"
                              style={{
                                width: visibleItems.has(2)
                                  ? `${
                                      (analyzeResult.breakdown.nftScore /
                                        1000) *
                                      100
                                    }%`
                                  : "0%",
                        ...
                        >
                          ...
                          <div className="mt-2 h-2 rounded-full bg-white/5">
                            <div
                              className="h-full rounded-full bg-gradient-to-r from-[#42E8E0] to-[#3BA3FF] transition-all duration-700"
                              style={{
                                width: visibleItems.has(3)
                                  ? `${
                                      (analyzeResult.breakdown.defiScore /
                                        1000) *
                                      100
                                    }%`
                                  : "0%",
                        ...
                        >
                          ...
                          <div className="mt-2 h-2 rounded-full bg-white/5">
                            <div
                              className="h-full rounded-full bg-gradient-to-r from-[#2E7FFF] to-[#6B4DFF] transition-all duration-700"
                              style={{
                                width: visibleItems.has(4)
                                  ? `${
                                      (analyzeResult.breakdown.contractScore /
                                        1000) *
                                      100
                                    }%`
                                  : "0%",
                    ...
                    <section className="rounded-[32px] border border-white/5 bg-[#030b1e]/90 p-12 backdrop-blur">
                      <div className="text-center">
                        ...
                        <p className="text-xs text-gray-500">
                          Enter a wallet address and click Analyze to see
                          reputation results
                        </p>
                      </div>
                    </section>
                ...
                <div className="sticky top-28 flex h-fit flex-col gap-6">
                  <section className="rounded-[28px] border border-white/10 bg-[#030b1e]/90 p-6 backdrop-blur">
                    <div className="flex items-center justify-between">
                      ...
                      <button className="text-sm text-[#7BDFFF]">
                        See all
                      </button>
                    </div>
                    <div className="mt-4 space-y-3">
